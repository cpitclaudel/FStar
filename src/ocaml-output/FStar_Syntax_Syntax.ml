...
let gen_reset:
  ...
  let reset uu____6193 = FStar_ST.op_Colon_Equals x (Prims.parse_int "0") in
  (gen1, reset)
  ...
  fun s  ->
    fun r  ->
      fun t  ->
        ...
        let uu____6270 = next_id () in
        { ppname = id; index = uu____6270; sort = t }
...
let freshen_bv: bv -> bv =
  fun bv  ->
    let uu____6287 = is_null_bv bv in
    if uu____6287
    then
      let uu____6288 =
        let uu____6291 = range_of_bv bv in
        FStar_Pervasives_Native.Some uu____6291 in
      new_bv uu____6288 bv.sort
    else
      (let uu___96_6293 = bv in
       let uu____6294 = next_id () in
       {
         ppname = (uu___96_6293.ppname);
         index = uu____6294;
         sort = (uu___96_6293.sort)
       })
let new_univ_name:
  ...
  fun ropt  ->
    ...
    let uu____6304 =
      let uu____6309 =
        let uu____6310 = FStar_Util.string_of_int id in
        Prims.strcat FStar_Ident.reserved_prefix uu____6310 in
      (uu____6309, (range_of_ropt ropt)) in
    FStar_Ident.mk_ident uu____6304
...
let set_bv_range: bv -> FStar_Range.range -> bv =
  fun bv  ->
    fun r  ->
      let uu___97_6416 = bv in
      {
        ppname = (FStar_Ident.mk_ident (((bv.ppname).FStar_Ident.idText), r));
        index = (uu___97_6416.index);
        sort = (uu___97_6416.sort)
      }
let lid_as_fv:
  FStar_Ident.lident ->
  ...
  fun l  ->
    fun dd  ->
      fun dq  ->
        let uu____6433 = withinfo l (FStar_Ident.range_of_lid l) in
        { fv_name = uu____6433; fv_delta = dd; fv_qual = dq }
...
let fvar:
  ...
  fun l  ->
    fun dd  ->
      fun dq  -> let uu____6454 = lid_as_fv l dd dq in fv_to_tm uu____6454
let lid_of_fv: fv -> FStar_Ident.lid = fun fv  -> (fv.fv_name).v
let range_of_fv: fv -> FStar_Range.range =
  fun fv  ->
    let uu____6463 = lid_of_fv fv in FStar_Ident.range_of_lid uu____6463
let set_range_of_fv: fv -> FStar_Range.range -> fv =
  fun fv  ->
    fun r  ->
      let uu___98_6472 = fv in
      let uu____6473 =
        let uu___99_6474 = fv.fv_name in
        let uu____6475 =
          let uu____6476 = lid_of_fv fv in
          FStar_Ident.set_lid_range uu____6476 r in
        { v = uu____6475; p = (uu___99_6474.p) } in
      {
        fv_name = uu____6473;
        fv_delta = (uu___98_6472.fv_delta);
        fv_qual = (uu___98_6472.fv_qual)
      }
let has_simple_attribute: term Prims.list -> Prims.string -> Prims.bool =
  fun l  ->
    fun s  ->
      FStar_List.existsb
        (fun uu___94_6496  ->
           match uu___94_6496 with
           | { n = Tm_constant (FStar_Const.Const_string (data,uu____6500));
               pos = uu____6501; vars = uu____6502;_} when data = s -> true
           | uu____6505 -> false) l
